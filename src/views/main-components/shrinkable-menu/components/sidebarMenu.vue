  ...
  <Menu ref="sideMenu" accordion :active-name="$route.name" :open-names="singleOpenName" :theme="menuTheme" width="auto"
    ...
    <template v-for="item in menuList">
      ...
      <template v-if="item.level == '1' && !item.showAlways">
        ...
        <span class="font_family icon_home"  v-if="itemTitle(item.children[0]) === '商务室首页'">&#xe661;</span>
        <span class="font_family icon_home"  v-if="itemTitle(item.children[0]) === '首页'">&#xe661;</span>
        <span class="font_family icon_home"  v-if="itemTitle(item.children[0]) === '商机学院'">&#xe664;</span>
        ...
        <Submenu v-if="item.children.length > 1" :name="item.name" :key="item.name">
          <template slot="title">
            <span class="font_family" v-if="itemTitle(item) === '我的关注'">&#xe679;</span>
          ...
          <template v-for="child in item.children">
            <template v-if="child.name == 'subscripManage'">
              <Submenu name="subscripManage" class="subBrowse">
                <template slot="title">
                  <span style="font-size: 18px">订阅管理</span>
                </template>
                <MenuItem name="addSub"><span style="font-size: 18px">新建订阅</span></MenuItem>
                <MenuItem name="editSub"><span style="font-size: 18px">修改订阅</span></MenuItem>
              </Submenu>
            </template>
            <MenuItem v-show="!(isFreeMember && (child.name == 'myExport' || child.name == 'myShare' || child.name == 'myProject'))" v-else :name="child.name" :key="'menuitem' + child.name">
              <span class="empty"></span>
              <Icon :type="child.icon" :size="iconSize" :key="'icon' + child.name"
                    v-if="showIconFlag(itemTitle(child))">
              </Icon>
      ...
      <template v-else>
        <Submenu :name="item.name" :key="item.name">
          <template slot="title">
            ...
            <span class="font_family" v-if="itemTitle(item) === '账号安全'">&#xe667;</span>
            <span class="font_family" v-if="itemTitle(item) === '我的应用'">&#xe66f;</span>
